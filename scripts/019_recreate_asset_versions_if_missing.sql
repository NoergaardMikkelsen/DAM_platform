-- Recreate asset_versions table if it does not exist (idempotent)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1
    ...
    -- Only apply RLS policies if helper functions exist
    IF EXISTS (
      SELECT 1 FROM pg_proc WHERE proname = 'has_client_access'
    ) AND EXISTS (
      SELECT 1 FROM pg_proc WHERE proname = 'get_user_role_key'
    ) THEN
